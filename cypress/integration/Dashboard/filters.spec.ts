...
describe('Test Suites Content Table', () => {
  ...
  it('should apply labels filters', () => {
    ...
    cy.get('[data-cy=labels-filter-dropdown]').should('be.visible');

    cy.get('[data-cy=key-input-0]').first().type('app');
    cy.get('[data-cy=value-input-0]').first().type('sites');
    cy.get('[data-cy=labels-filter-dropdown]').contains('Ok').click();

    cy.get('[data-cy=labels-filter-dropdown]').should('not.be.visible');

    cy.intercept(`${baseEndpointUrl}/test-suite-with-executions?*`, req => {
      expect(req.url).to.have.string(`selector=${key}%3D${value}`);
    }).as('filter request');

    cy.wait('@filter request');
  });
});
