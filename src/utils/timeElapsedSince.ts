...
export const timeElapsedSince = (now: number) => (past: number): TimeFormat => {
  ...
  if (durationInSeconds >= FEW_SECONDS && durationInSeconds < MINUTE) return {
    long: 'a minute ago',
    short: '1 m',
  }

  // within 59 minutes
  if (MINUTE >= 60 && durationInSeconds < HOUR) return {
    long: `${Math.floor(durationInSeconds / MINUTE)} minutes ago`,
    short: `${Math.floor(durationInSeconds / MINUTE)} m`,
  }

  // 1 hour
  if (durationInSeconds >= HOUR && durationInSeconds < TWO_HOURS) return {
    long: `1 hour ago`,
    short: `1 h`,
  }

  // hours
  if (durationInSeconds >= TWO_HOURS && durationInSeconds < DAY) return {
    long: `${Math.floor(durationInSeconds / HOUR)} hours ago`,
    short: `${Math.floor(durationInSeconds / HOUR)} h`,
  }

  // day
  if (durationInSeconds >= DAY && durationInSeconds < TWO_DAYS) return {
    long: `yesterday`,
    short: `1 d`,
  }

  // days
  if (durationInSeconds >= TWO_DAYS && durationInSeconds < MONTH) return {
