...
export const timeElapsedSince =
  (now: Date) =>
  (past: Date): TimeFormat => {
    ...
    const durationInSeconds = differenceInSeconds(now, past);
    ...
    return {
      long: `${format(past, 'MMMM d, yyyy')}`,
      short: `${format(past, 'MMM d')}`,
    };
  };
